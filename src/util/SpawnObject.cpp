...
namespace YimMenu
{
    Entity SpawnObjectEntity(uint32_t hash, Vector3 coords, float heading)
    {
        ...
        if (heading != 0.f)
            ENTITY::SET_ENTITY_ROTATION(obj, 0.f, 0.f, heading, 2, true);
    ...
    {
        FiberPool::Push([=] {
            (void)SpawnObjectEntity(hash, coords);
        });
    }
}
