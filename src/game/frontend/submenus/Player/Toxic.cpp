...
namespace YimMenu::Submenus
{
	std::shared_ptr<Category> BuildToxicMenu()
	{
		...
		auto harassment = std::make_shared<Group>("Harassment");
		harassment->AddItem(std::make_shared<PlayerCommandItem>("ragdollplayer"_J));
		harassment->AddItem(std::make_shared<PlayerCommandItem>("ufoabduction"_J));
		harassment->AddItem(std::make_shared<PlayerCommandItem>("removeplayerweapons"_J));
		harassment->AddItem(std::make_shared<PlayerCommandItem>("spawnnpcattackers"_J));
		harassment->AddItem(std::make_shared<PlayerCommandItem>("spawnanimalattackers"_J));
		harassment->AddItem(std::make_shared<PlayerCommandItem>("shadowclonestalker"_J));

		menu->AddItem(general);
		menu->AddItem(events);
		menu->AddItem(mount);
		menu->AddItem(vehicle);
		menu->AddItem(harassment);
